...
export const ActiveWorkout = ({ 
  ...
  useEffect(() => {
    if (!editingWorkout && session) {
      onUpdateSession({ ...session, isStarted, exercises, name: sessionName });
    }
  }, [exercises, isStarted, sessionName]);
  ...
  const moveExercise = (index, direction) => {
    ...
    setExercises(newEx);
    persistTemplateUpdate(sessionName, newEx);
  };

  const persistTemplateUpdate = (newName, newExList) => {
    if (!isStarted && session?.templateId) {
      onUpdateTemplate({
        ...session,
        id: session.templateId,
        name: newName,
        exercises: newExList.map(ex => ex.exerciseId)
      });
    }
  };
  ...
  const toggleNote = (setId) => {
    setExpandedNotes(prev => ({ ...prev, [setId]: !prev[setId] }));
  };
  ...
  return html`
    <div className="fixed inset-0 z-50 bg-slate-950 flex flex-col pt-safe">
      ...
      <!-- HEADER -->
      ${!isStarted ? html`
        <header className="p-4 border-b border-slate-800 bg-slate-900 flex items-center justify-between shadow-lg shrink-0">
          ...
          <h2 className="font-bold text-white text-sm uppercase tracking-widest">Planung</h2>
      ...
      <main className="flex-1 overflow-y-auto p-3 space-y-3 pb-32">
        ...
        <!-- INFO CARD (Only Preparation) -->
        ${!isStarted && html`
          <div className="bg-slate-900 p-4 rounded-3xl border border-slate-800 mb-1">
            <div className="flex items-center justify-between group">
              ${isEditingName ? html`
                <input 
                  ...
                  autoFocus
                  className="bg-slate-950 border border-emerald-500 rounded-xl px-3 py-1 text-xl font-black text-white w-full outline-none"
                />
              ` : html`
                <h1 onClick=${() => setIsEditingName(true)} className="text-xl font-black text-white flex items-center gap-2 cursor-pointer">
                  ${sessionName} <${Edit2} size=${16} className="text-slate-600 opacity-0 group-hover:opacity-100 transition-opacity" />
                </h1>
              `}
            </div>
            <p className="text-slate-500 text-[9px] uppercase font-bold tracking-widest mt-1">Reihenfolge anpassen oder Übungen hinzufügen</p>
          </div>
        `}
            ...
            return html`
              <div key=${exIdx} className="flex items-center gap-2 bg-slate-900 p-2.5 rounded-xl border border-slate-800 shadow-sm transition-all">
                ...
                <button onClick=${() => {
                  ...
                  persistTemplateUpdate(sessionName, newList);
                }} className="text-red-500/30 p-2 active:text-red-500 transition-colors"><${Trash2} size=${18} /></button>
              </div>
            `;
          } else {
            return html`
        ...
        <button onClick=${() => setShowAddExercise(true)} className=${`w-full border-2 border-dashed border-slate-800 rounded-xl font-bold flex items-center justify-center gap-2 active:bg-slate-900 transition-all ${!isStarted ? 'py-4 text-xs text-slate-500' : 'py-5 text-emerald-500 border-emerald-500/20 bg-emerald-500/5 text-xs uppercase tracking-widest'}`}>
            <${Plus} size=${!isStarted ? 16 : 18} /> Übung hinzufügen
        </button>
      </main>
            ...
            <div className="p-4 overflow-y-auto space-y-2 pb-10">
              ${allExercises.sort((a,b)=>a.name.localeCompare(b.name)).map(ex => html`
                <button key=${ex.id} onClick=${() => {
                  const newList = [...exercises, { exerciseId: ex.id, sets: [{ id: generateId(), weight: 0, reps: 0, rir: 0, completed: false, note: '' }] }];
                  setExercises(newList);
                  persistTemplateUpdate(sessionName, newList);
